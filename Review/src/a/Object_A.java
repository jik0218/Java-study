...
// - 객체의 예시
// EX) 커피 자판기
// > 속성: 커피의 종류, 가격, 재고, 물의 온도등
// > 기능: 커피 제조, 제고 관리, 가격 설정 등
...
public class Object_A {
	public static void main(String[] args) {
		...
		// ex) '전기 자동차' 클래스
		//       - 일반 자동차 클래스의 모든 특성을 상속 받고
		//       - 추가적으로 배터리 관련 속성과 충전 메서드를 가짐
		
		// 4. 다형성(Polymorphisim)
		// : 같은 메서드 호출이 다른 클래스의 객체에 다른 방식으로 실행 가능
		// ex) '주행'이라는 메서드(행동)는 '전기 자동차' 객체에서는 전기를 사용
		//          '가솔린자동차' 객체에서는 가솔린을 사용하여 주행
		
		// 5. 캡슐화(Encapsulation)
		// : 객체의 데이터를 외부로부터 숨기고, 내부의 메서드를 통해서만
		// : 해당 데이터를 조작할 수 있게하는 것
		// ex) 자동차의 엔진은 외부에서 직접 조작 X
		//      엔전을 조작하기 위해서는 시동 버튼, 가속 페달 등을 사용
